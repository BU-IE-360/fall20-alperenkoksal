...
# I didn't omit the na values in this data because na values in those rows and columns was not problem for me and I didn't need them while i need the other values in those columns and rows.

# only take the data with richter > 5 and date between 2017 and 2009

deprem <- subset(deprem,deprem["richter"]>5.0)

# only take the years as date
i = 1
...
# when we plot the histogram we see that in 2011 and 2017 there wwere big earthquakes which have richter >= 6.5 and in 2012 2013 2014 there wwere moderatly big earthquakes which have richter >= 6.0

ggplot(deprem, aes(richter,)) + geom_histogram(binwidth = 0.1) + facet_grid(date~.)

# when we comparte the search volume "deprem" with the real data of earthquakes we see that when the trend is before the most things searching and verifying the occurrence of the big earthquakes that have higher richter values and have bigger impacts.
...
# omit the na values in date
dolar <- na.omit(dolar)

# only take the years and months as date.
i = 1
...
# as we see from histogram dollar Turkish Lira rate is increasing almost constantly from first month of 2020 to now and nowadays the rate has some fluctuations and decrease in value ( maybe the decrease cant be observed very clearly because of the missing data).

ggplot(dolar, aes(kur,)) + geom_histogram(binwidth = 0.1) + facet_grid(Tarih~.)
...
ggplot(faizyuzde, aes(Faiz,)) + geom_histogram(binwidth = 0.05) + facet_grid(Tarih~.)


# while interest rates are decreasing there is a trend of searching for the bank loans.
